...
class ScreenState extends StatefulWidget {
  ...
  ScreenState(this.IdCity,this.NameCity);

  @override
  State<ScreenState> createState() => _ScreenScreenState();
}

class _ScreenScreenState extends State<ScreenState> {
  var Baseurl="";
                              ...
                              child: Directionality(
                                ...
                                child: TextField(
                                  onChanged: (val) async {
                                    if(val.isNotEmpty)
                                    {
                                       var ss=await ApiService.GetRegion(Baseurl, UserName, Password,widget.IdCity,val.toString());
                                      setState(() {
                                        if(ss.res.length>0)
                                        {
                                          datamian=ss.res;
                                  ...
                                  decoration: InputDecoration(
                                      ...
                                      hintText: 'منطقه خود را جستجو کنید...'
                                  ),
                                ),
                              ),
                            ),
                                  ...
                                  child: Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        Expanded(
                                            flex: 50,
                                            child: BoxInfo_3('منطقه',datamian[item].name)),
                                        Container(
                                          width: 2,
                                          height: Sizewid*1/7,
                                          color: ColorLine,
